...
campaign_delay_pop_size_prop_teams_sensitivity <- sim_params_pop_size_prop_teams_sensitivity %>% 
    rowwise() %>%
    do({
        with(
            .,
...
## Remove some columns ==== 
campaign_delay_pop_size_prop_teams_sensitivity_trunc <- campaign_delay_pop_size_prop_teams_sensitivity %>% 
...
campaign_metrics_pop_size_prop_teams_sensitivity <- sim_params_pop_size_prop_teams_sensitivity %>%
    rowwise() %>%
    do({
        with(
            .,
...
## Remove some columns ==== 
campaign_metrics_pop_size_prop_teams_sensitivity_trunc <- campaign_metrics_pop_size_prop_teams_sensitivity %>% 
    select(-c(strategy, 
              location_id, 
              mt_equip_type
    )
...
pop_size_prop_teams_sensitivity_sc_analysis_full <- bind_cols(
    ...
    campaign_metrics_pop_size_prop_teams_sensitivity_trunc
        ...
        n_teams_fixed = sim_params_pop_size_prop_teams_sensitivity$n_teams_fixed,
        n_teams_mobile = sim_params_pop_size_prop_teams_sensitivity$n_teams_mobile
    )


#' Epi input: compounded delays
sc_analysis_pop_size_prop_teams_sensitivity_w_cpd_delays <- pop_size_prop_teams_sensitivity_sc_analysis_full %>%
...
sc_analysis_pop_size_prop_teams_sensitivity_summary <- sc_analysis_pop_size_prop_teams_sensitivity_w_cpd_delays %>%
    group_by(strategy, mt_equip_type, near_pop, far_pop, n_teams_fixed, n_teams_mobile) %>%
...
coverage_duration_plot_plain_shapes_beeswarm <- ggplot(data = sc_analysis_pop_size_prop_teams_sensitivity_summary, 
    ...
    scale_y_continuous(breaks = seq(min(sc_analysis_pop_size_prop_teams_sensitivity_summary$average_coverage), 
                                    ...
                                    length.out = 5),
                       labels = percent(seq(min(sc_analysis_pop_size_prop_teams_sensitivity_summary$average_coverage), 
                                            max(sc_analysis_pop_size_prop_teams_sensitivity_summary$average_coverage), 
                                            length.out = 5)
                       )
    ) +
...
saveRDS(sc_analysis_pop_size_prop_teams_sensitivity_w_cpd_delays, 
        file = "./model_output/deterministic_framework_analysis_output/sensitivity_analysis/pop_sizes/sc_analysis_pop_size_prop_teams_sensitivity_full.rds")


saveRDS(sc_analysis_pop_size_prop_teams_sensitivity_summary, 
        file = "./model_output/deterministic_framework_analysis_output/sensitivity_analysis/pop_sizes/sc_analysis_pop_size_prop_teams_sensitivity_summary.rds")
