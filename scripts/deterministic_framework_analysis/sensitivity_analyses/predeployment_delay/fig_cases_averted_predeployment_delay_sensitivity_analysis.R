...
ggsave(plot = cases_averted_predeployment_delay_sensitivity_analysis_plot,
       filename = './figures/deterministic_framework_analysis_figures/sensitivity_analysis/predeployment_delay/fig6_cases_averted_predeployment_delay_sensitivity_analysis_plot.jpg',
       width = 23.76,
       height = 17.86,
       units = 'cm'
)
...
cases_averted_df_cc_mod %>% 
    ...
    scale_y_continuous(breaks = round(seq(min(range(cases_averted_df_cc_mod$relative_cases_averted)[1]/1000),
                                          max(range(cases_averted_df_cc_mod$relative_cases_averted)[2]/1000),
                                          2
    ), 1),
    labels = round(seq(min(range(cases_averted_df_cc_mod$relative_cases_averted)[1]/1000),
                       max(range(cases_averted_df_cc_mod$relative_cases_averted)[2]/1000),
                       2
    ), 1)
    ) +
