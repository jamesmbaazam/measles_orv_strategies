...
initializePop <- function(N, init_prop_immune, I0) {
  immune <- floor((N - I0) * init_prop_immune)
  init_pop <- data.frame(
    Sus1 = N - immune - I0,
    Sus2 = 0,
    Exp1 = 0,
  ...
  if(any(init_pop < 0)){
    stop('Check your inputs; one of the states is negative')
  }else{
      return(init_pop)
    }
}
...
runSimulations <- function(R0 # transmission coefficient
  ...
  while (time < run_time) {
    ...
    if ((!is.na(vaxDay) & time < vaxDay) | (time > vaxDay + orv_duration - 1)) {
      simResults <- rbind(simResults, data.frame(time, step(pop = simResults[time, -1], R0 = R0)))
    }else if (!is.na(vaxDay) & (time >= vaxDay & time <= (vaxDay + orv_duration - 1))){
